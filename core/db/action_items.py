...
def list_open(user_id: int, priority: Optional[Priority] = None) -> list[dict]:
    sql = "SELECT * FROM action_items WHERE user_id = ? AND is_completed = 0"
    params: list = [user_id]
    if priority:
        sql += " AND priority = ?"
        params.append(priority)
    sql += " ORDER BY priority ASC, target_date ASC NULLS LAST, id ASC"
    with connect() as conn:
...
def mark_completed(item_id: int) -> None:
    with connect() as conn:
        conn.execute("UPDATE action_items SET is_completed = 1 WHERE id = ?", (item_id,))
